...
function mask(value, precision) {
    if (!precision)
        precision = 2;
    var returnVal = $('<input>', { type: 'hidden' }).maskMoney({ precision: precision }).maskMoney('mask', value).val();
    return returnVal;
}

function unmask(maskedValue) {
    var returnVal = $('<input>', { type: 'hidden', value: maskedValue }).maskMoney('unmasked')[0];
    return returnVal;
}